# Instructions

- Following Playwright test failed.
- Explain why, be concise, respect Playwright best practices.
- Provide a snippet of code with the fix, if possible.

# Test info

- Name: institution\reports.spec.ts >> UC-IN-04: Institution Reports >> email report sends to recipient (Resend stubbed)
- Location: playwright\institution\reports.spec.ts:119:7

# Error details

```
Test timeout of 30000ms exceeded.
```

```
Error: locator.click: Test timeout of 30000ms exceeded.
Call log:
  - waiting for getByRole('button', { name: /send pdf/i })
    - locator resolved to <button disabled data-lov-name="Button" data-component-line="361" data-component-name="Button" data-component-file="Reports.tsx" data-lov-id="src\pages\institution\Reports.tsx:361:14" data-component-path="src\pages\institution\Reports.tsx" data-component-content="%7B%22text%22%3A%22Send%20PDF%22%2C%22className%22%3A%22w-full%20gap-2%22%7D" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none …>…</button>
  - attempting click action
    2 × waiting for element to be visible, enabled and stable
      - element is not enabled
    - retrying click action
    - waiting 20ms
    2 × waiting for element to be visible, enabled and stable
      - element is not enabled
    - retrying click action
      - waiting 100ms
    55 × waiting for element to be visible, enabled and stable
       - element is not enabled
     - retrying click action
       - waiting 500ms

```

# Page snapshot

```yaml
- generic [ref=e2]:
  - region "Notifications (F8)":
    - list
  - region "Notifications alt+T"
  - generic [ref=e4]:
    - complementary [ref=e5]:
      - generic [ref=e6]:
        - img [ref=e8]
        - generic [ref=e11]:
          - generic [ref=e12]: Zoomiez
          - generic [ref=e13]: Powered By Zoomiez
      - navigation [ref=e14]:
        - link "Dashboard" [ref=e15] [cursor=pointer]:
          - /url: /institution/dashboard
          - img [ref=e16]
          - generic [ref=e21]: Dashboard
        - link "Teacher Management" [ref=e22] [cursor=pointer]:
          - /url: /institution/teachers
          - img [ref=e23]
          - generic [ref=e28]: Teacher Management
        - link "Student Analytics" [ref=e29] [cursor=pointer]:
          - /url: /institution/students
          - img [ref=e30]
          - generic [ref=e32]: Student Analytics
        - link "Teacher Progress" [ref=e33] [cursor=pointer]:
          - /url: /institution/progress
          - img [ref=e34]
          - generic [ref=e37]: Teacher Progress
        - link "Reports" [ref=e38] [cursor=pointer]:
          - /url: /institution/reports
          - img [ref=e39]
          - generic [ref=e42]: Reports
        - link "Settings" [ref=e43] [cursor=pointer]:
          - /url: /institution/settings
          - img [ref=e44]
          - generic [ref=e47]: Settings
      - button [ref=e49] [cursor=pointer]:
        - img [ref=e50]
    - generic [ref=e52]:
      - banner [ref=e53]:
        - generic [ref=e54]:
          - button "Switch to dark mode" [ref=e55] [cursor=pointer]:
            - img [ref=e56]
          - button "IN" [ref=e58] [cursor=pointer]:
            - generic [ref=e60]: IN
            - img [ref=e61]
      - main [ref=e63]:
        - generic [ref=e64]:
          - generic [ref=e65]:
            - heading "PDF Export - Reports & Logs" [level=1] [ref=e66]
            - paragraph [ref=e67]: Generate, preview, download, and share AI-powered reports for students, teachers, and parents.
          - generic [ref=e68]:
            - heading "PDF Generation Controls" [level=3] [ref=e70]
            - generic [ref=e71]:
              - generic [ref=e72]:
                - generic [ref=e73]:
                  - text: Report Type
                  - combobox [ref=e74] [cursor=pointer]:
                    - generic: Student
                    - img [ref=e75]
                - generic [ref=e77]:
                  - text: Student
                  - combobox [ref=e78] [cursor=pointer]:
                    - generic: Select student
                    - img [ref=e79]
                - generic [ref=e81]:
                  - text: Subject
                  - combobox [ref=e82] [cursor=pointer]:
                    - generic: All Subjects
                    - img [ref=e83]
                - generic [ref=e85]:
                  - text: Class
                  - combobox [ref=e86] [cursor=pointer]:
                    - generic: All Classes
                    - img [ref=e87]
                - generic [ref=e89]:
                  - text: Date From
                  - textbox [ref=e90]
                - generic [ref=e91]:
                  - text: Date To
                  - textbox [ref=e92]
                - generic [ref=e93]:
                  - text: Detail Level
                  - combobox [ref=e94] [cursor=pointer]:
                    - generic: Summary
                    - img [ref=e95]
                - generic [ref=e98]:
                  - switch [disabled] [ref=e99]
                  - generic [ref=e100]: Include question-level feedback
              - generic [ref=e101]:
                - button "Preview" [disabled]:
                  - img
                  - text: Preview
                - button "Download PDF" [disabled]:
                  - img
                  - text: Download PDF
                - button "Print Report" [disabled]:
                  - img
                  - text: Print Report
          - generic [ref=e102]:
            - generic [ref=e104]:
              - img [ref=e106]
              - generic [ref=e109]:
                - paragraph [ref=e110]: Student Report
                - paragraph [ref=e111]: Detailed AI evaluation report with topic and question-level insights.
              - button "Use This Type" [ref=e112] [cursor=pointer]:
                - img
                - text: Use This Type
            - generic [ref=e114]:
              - img [ref=e116]
              - generic [ref=e121]:
                - paragraph [ref=e122]: Teacher Report
                - paragraph [ref=e123]: Class performance, pass rate, and aggregated student insight report.
              - button "Use This Type" [ref=e124] [cursor=pointer]:
                - img
                - text: Use This Type
            - generic [ref=e126]:
              - img [ref=e128]
              - generic [ref=e130]:
                - paragraph [ref=e131]: Parent Report
                - paragraph [ref=e132]: Simple and readable student summary for parent communication.
              - button "Use This Type" [ref=e133] [cursor=pointer]:
                - img
                - text: Use This Type
          - generic [ref=e134]:
            - heading "Send Report via Email" [level=3] [ref=e136]
            - generic [ref=e137]:
              - generic [ref=e138]:
                - generic [ref=e139]:
                  - text: Recipient Email
                  - textbox "parent@example.com" [active] [ref=e140]
                - generic [ref=e141]:
                  - button "Send PDF" [disabled]:
                    - img
                    - text: Send PDF
              - paragraph [ref=e142]: "Current selection: Student report, summary detail."
          - generic [ref=e143]:
            - heading "Preview" [level=3] [ref=e145]
            - paragraph [ref=e147]: Generate a preview to inspect data before downloading.
          - generic [ref=e148]:
            - heading "Export History" [level=3] [ref=e150]
            - table [ref=e154]:
              - rowgroup [ref=e155]:
                - row "Type Target Action Status Recipient Created" [ref=e156]:
                  - columnheader "Type" [ref=e157]
                  - columnheader "Target" [ref=e158]
                  - columnheader "Action" [ref=e159]
                  - columnheader "Status" [ref=e160]
                  - columnheader "Recipient" [ref=e161]
                  - columnheader "Created" [ref=e162]
              - rowgroup [ref=e163]:
                - row "No export history yet." [ref=e164]:
                  - cell "No export history yet." [ref=e165]
```

# Test source

```ts
  24  |     await page.route(`${API}/api/institution/reports/preview`, (r) =>
  25  |       r.fulfill({
  26  |         json: {
  27  |           report_type: 'student',
  28  |           title: 'Student Performance Report — Ahmed Khan',
  29  |           generated_at: new Date().toISOString(),
  30  |           profile: { name: 'Ahmed Khan', email: 'enrolled-student@example.com', class_level: 'O-Level', subject: 'Chemistry' },
  31  |           summary: { total_tests: 12, average_score: 72, pass_rate: '83%' },
  32  |           topic_breakdown: [{ topic: 'Atomic Structure', percentage: 85 }, { topic: 'Electrolysis', percentage: 60 }],
  33  |           insights: {
  34  |             common_strengths: ['Atomic structure'],
  35  |             common_weaknesses: ['Electrolysis'],
  36  |             simple_feedback: 'Ahmed shows strong conceptual understanding.',
  37  |           },
  38  |           recommendations: ['Review electrolysis discharge potential rules'],
  39  |         },
  40  |       })
  41  |     );
  42  |     await page.route(`${API}/api/institution/reports/download`, (r) =>
  43  |       r.fulfill({
  44  |         body: '%PDF-1.4 fake content',
  45  |         headers: { 'Content-Type': 'application/pdf', 'Content-Disposition': 'attachment; filename="report.pdf"' },
  46  |       })
  47  |     );
  48  |     // SENSITIVE: Resend stub
  49  |     await page.route(`${API}/api/institution/reports/email`, (r) =>
  50  |       r.fulfill({ json: { message: 'Report emailed successfully' } })
  51  |     );
  52  |     await page.route(`${API}/api/institution/reports/history`, (r) =>
  53  |       r.fulfill({ json: [] })
  54  |     );
  55  |   });
  56  | 
  57  |   test('reports page renders controls', async ({ page }) => {
  58  |     await page.goto('/institution/reports');
  59  |     await expect(page.getByText(/PDF.*Reports|PDF Generation/i)).toBeVisible({ timeout: 8_000 });
  60  |     await expect(page.getByRole('button', { name: /preview/i })).toBeVisible();
  61  |     await expect(page.getByRole('button', { name: /download pdf/i })).toBeVisible();
  62  |     await expect(page.getByRole('button', { name: /print report/i })).toBeVisible();
  63  |   });
  64  | 
  65  |   test('preview renders KPIs and topic breakdown', async ({ page }) => {
  66  |     await page.goto('/institution/reports');
  67  |     // Select student target
  68  |     await page.getByRole('combobox', { name: /student/i }).selectOption?.('enrolled-student@example.com');
  69  |     const studentSelect = page.locator('select').first();
  70  |     if (await studentSelect.isVisible()) await studentSelect.selectOption('enrolled-student@example.com');
  71  | 
  72  |     await page.getByRole('button', { name: /preview/i }).click();
  73  |     await expect(page.getByText(/Ahmed Khan/i)).toBeVisible({ timeout: 8_000 });
  74  |     await expect(page.getByText(/total_tests|12/i)).toBeVisible({ timeout: 5_000 });
  75  |     await expect(page.getByText(/Atomic Structure/i)).toBeVisible();
  76  |     await expect(page.getByText(/Ahmed shows strong conceptual/i)).toBeVisible();
  77  |   });
  78  | 
  79  |   test('print button disabled before preview', async ({ page }) => {
  80  |     await page.goto('/institution/reports');
  81  |     const printBtn = page.getByRole('button', { name: /print report/i });
  82  |     await expect(printBtn).toBeDisabled({ timeout: 8_000 });
  83  |   });
  84  | 
  85  |   test('print button opens new tab after preview', async ({ page, context }) => {
  86  |     await page.goto('/institution/reports');
  87  |     // Get a preview first
  88  |     const studentSelect = page.locator('select').first();
  89  |     if (await studentSelect.isVisible()) await studentSelect.selectOption('enrolled-student@example.com');
  90  |     await page.getByRole('button', { name: /preview/i }).click();
  91  |     await expect(page.getByText(/Ahmed Khan/i)).toBeVisible({ timeout: 8_000 });
  92  | 
  93  |     const [printPage] = await Promise.all([
  94  |       context.waitForEvent('page'),
  95  |       page.getByRole('button', { name: /print report/i }).click(),
  96  |     ]);
  97  |     await printPage.waitForLoadState();
  98  |     await expect(printPage.url()).toContain('/institution/reports/print');
  99  |     await printPage.close();
  100 |   });
  101 | 
  102 |   test('print page renders institution branding', async ({ page, context }) => {
  103 |     await page.goto('/institution/reports');
  104 |     const studentSelect = page.locator('select').first();
  105 |     if (await studentSelect.isVisible()) await studentSelect.selectOption('enrolled-student@example.com');
  106 |     await page.getByRole('button', { name: /preview/i }).click();
  107 |     await expect(page.getByText(/Ahmed Khan/i)).toBeVisible({ timeout: 8_000 });
  108 | 
  109 |     const [printPage] = await Promise.all([
  110 |       context.waitForEvent('page'),
  111 |       page.getByRole('button', { name: /print report/i }).click(),
  112 |     ]);
  113 |     await printPage.waitForLoadState();
  114 |     // Should show institution info from localStorage
  115 |     await expect(printPage.getByText(/powered by zoomiez/i)).toBeVisible({ timeout: 8_000 });
  116 |     await printPage.close();
  117 |   });
  118 | 
  119 |   test('email report sends to recipient (Resend stubbed)', async ({ page }) => {
  120 |     await page.goto('/institution/reports');
  121 |     const studentSelect = page.locator('select').first();
  122 |     if (await studentSelect.isVisible()) await studentSelect.selectOption('enrolled-student@example.com');
  123 |     await page.getByPlaceholder(/recipient email|parent@/i).fill('parent@example.com');
> 124 |     await page.getByRole('button', { name: /send pdf/i }).click();
      |                                                           ^ Error: locator.click: Test timeout of 30000ms exceeded.
  125 |     await expect(page.getByText(/emailed successfully|sent/i)).toBeVisible({ timeout: 8_000 });
  126 |   });
  127 | });
  128 | 
  129 | test.describe('UC-IN-02: Teacher Management', () => {
  130 |   test.beforeEach(async ({ page }) => {
  131 |     await page.route(`${API}/api/institution/teachers`, (r) =>
  132 |       r.fulfill({
  133 |         json: [
  134 |           { _id: 'tch-001', email: 'teacher@example.com', name: 'Test Teacher', status: 'active', subjects: ['Chemistry'], quiz_count: 5 }
  135 |         ],
  136 |       })
  137 |     );
  138 |     // SENSITIVE: Resend stub
  139 |     await page.route(`${API}/api/institution/teachers/invite`, (r) =>
  140 |       r.fulfill({ json: { message: 'Teacher invited' } })
  141 |     );
  142 |   });
  143 | 
  144 |   test('teacher list renders', async ({ page }) => {
  145 |     await page.goto('/institution/teachers');
  146 |     await expect(page.getByText(/Test Teacher/i)).toBeVisible({ timeout: 8_000 });
  147 |   });
  148 | 
  149 |   test('invite teacher (Resend stubbed)', async ({ page }) => {
  150 |     await page.goto('/institution/teachers');
  151 |     const inviteBtn = page.getByRole('button', { name: /invite/i });
  152 |     if (await inviteBtn.isVisible()) {
  153 |       await inviteBtn.click();
  154 |       await page.getByLabel(/email/i).fill('new-teacher@example.com');
  155 |       await page.getByLabel(/name/i).fill('New Teacher');
  156 |       await page.getByRole('button', { name: /send invite|confirm/i }).click();
  157 |       await expect(page.getByText(/invited|success/i)).toBeVisible({ timeout: 8_000 });
  158 |     }
  159 |   });
  160 | });
  161 | 
```